...
{
    // Obtenemos los parámetros de filtro del request
    $query = PersonalInformation::query();

    // Filtrar por título del perfil
    if ($request->filled('profile_title')) {
    ...
    $filtered_users = $query->get();

    // Retornamos la vista con los resultados filtrados
    return view('filter', ['users_data' => $filtered_users]);
}
/////////////

public function calendar()
{
    // Obtener las entrevistas de la base de datos o de la sesión
    $interviews = session('interviews', []); // Cambiar esto para usar la base de datos en lugar de sesión

    // Retornar la vista del calendario, pasando las entrevistas
    return view('calendar', compact('interviews'));
}

///////////////////77777
// Método para almacenar una nueva entrevista
public function storeC (Request $request)
{
    ...
    // Almacenar las entrevistas actualizadas en la sesión
    session(['interviews' => $interviews]);

    // Redirigir de vuelta al calendario
    return redirect()->route('calendar'); // Asegúrate de tener la ruta 'calendar' definida
}
...
public function destroyC($id)
{
    ...
    // Filtrar las entrevistas para eliminar la seleccionada
...
public function storeV(Request $request)
{
    ...
    // Obtener las vacantes de la sesión
    $vacancies = session('vacancies', []);

    // Agregar la nueva vacante
    $vacancies[] = [
        'id' => count($vacancies) + 1, // Generar un ID simple
        'title' => $validated['title'], // Agregar el título
        'date' => $validated['date'],
        'description' => $validated['description'],
    ];

    // Almacenar las vacantes actualizadas en la sesión
    session(['vacancies' => $vacancies]);

    // Redirigir de vuelta a la lista de vacantes
    return redirect()->route('vacancies.index'); // Asegúrate de que esta ruta esté definida
}
...
public function destroyV($id)
{
    ...
    // Filtrar las vacantes para eliminar la que tenga el ID especificado
    $vacancies = array_filter($vacancies, fn($vacancy) => $vacancy['id'] != $id);

    // Almacenar la lista actualizada de vacantes en la sesión
    session(['vacancies' => array_values($vacancies)]);
